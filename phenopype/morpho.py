...
class landmark_module:
    ...
    def draw(self, image_path, image_name, **kwargs):
        
        # initialize -----
        self.image = cv2.imread(image_path)
        self.filename = image_name
        image_diag = (self.image.shape[0]+self.image.shape[1])/2        
        size = kwargs.get("size", int(image_diag/(10 * 50)))
        ...
        while(not self.done):
            ...
            if self.idx > 0:
                ...
                self.df["idx"] = list(range(1,self.idx+1))
